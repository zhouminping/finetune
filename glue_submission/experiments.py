...
def MRPC_train(data_folder, output_folder, gpu_num):
    ...
    for x in train_X:
        if type(x[0]) != str or type(x[1]) != str:
            print(x)
    model = Comparison(low_memory_mode=True, batch_size=32, base_model_path="SummariesBase.jl", n_epochs=5, keep_best_model=True, val_set=(dev_X, dev_Y), visible_gpus=[gpu_num],
...
def STS_train(data_folder, output_folder, gpu_num):
    ...
    model = MultiFieldRegressor(low_memory_mode=True, batch_size=32, base_model_path="SummariesBase.jl", n_epochs=5, keep_best_model=True, val_set=(dev_X, dev_Y), visible_gpus=[gpu_num], regression_loss="L1", max_length=estimate_max_len(train_X))
...
def QQP_train(data_folder, output_folder, gpu_num):
    ...
    model = Comparison(low_memory_mode=True, batch_size=32, base_model_path="SummariesBase.jl", n_epochs=5, keep_best_model=True, val_set=(dev_X, dev_Y), visible_gpus=[gpu_num], max_length=estimate_max_len(train_X))
