...
def featurizer(X, encoder, dropout_placeholder, config, train=False, reuse=None, max_length=None):
    ...
    with tf.variable_scope('model/featurizer', reuse=reuse):
        ...
        if config.reduce_states.lower() == "sum":
            clf_h = tf.map_fn(lambda i: tf.reduce_sum(h[i, :pool_idx[i], :], axis=0), tf.range(shape_list(X)[0]), dtype=tf.float32)
        elif config.reduce_states.lower() == "mean":
            clf_h = tf.map_fn(lambda i: tf.reduce_mean(h[i, :pool_idx[i], :], axis=0), tf.range(shape_list(X)[0]), dtype=tf.float32)
        elif config.reduce_states.lower() == "final":
            ...
            raise Exception("Requires either sum, mean or final for reduce_states parameter")
